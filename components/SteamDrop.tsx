...
export default function SteamDrop() {
    ...
    return (
        <section id="steamdrop" className="w-full h-fit border-1 border-(--border) bg-(--section-back) max-[481px]:bg-transparent max-[481px]:border-0 rounded-2xl lg:px-8 px-3 py-4 max-[481px]:-mt-5 max-[481px]:gap-3 grid max-[481px]:grid-cols-1 grid-cols-2">
            <div className="flex flex-col gap-6">
                ...
                <span className="lg:text-base md:text-xs lg:w-130 md:w-85 lg:whitespace-wrap tracking-wide max-[481px]:hidden">Преврати пополнение кошелька в твой постоянный профит. Мы разыгрываем ценные призы, Steam-гифты и промокоды только для наших подписчиков. Все новости о самых низких
                комиссиях и специальных акциях — мгновенно в ленте!</span>
            </div>
            <div className="w-full max-[481px]:mt-2 grid lg:grid-cols-[repeat(4,126px)] min-[481px]:grid-cols-4 min-[320px]:grid-cols-3 lg:grid-rows-2 auto-rows-min lg:gap-x-3 gap-x-6 gap-y-4 max-[481px]:!justify-items-center justify-end justify-items-end lg:content-start content-center">
