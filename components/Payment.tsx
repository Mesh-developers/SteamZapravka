...
export default function Payment({ success }:PaymentProps) {
    ...
    return (
        <section className="relative overflow-hidden w-full flex flex-col gap-5 pl-15 py-10 max-[481px]:pl-0 max-[481px]:py-5 bg-(--section-back) border-1 border-(--border) rounded-3xl">
            ...
            <div className="relative z-5 flex flex-col gap-4 items-center">
                ...
                <h1 className={`text-4xl font-(family-name:--manrope-semibold) -mt-4 text-center ${!success ? "max-[481px]:text-3xl" : ""}`}>{success ? "Оплата прошла успешно" : "Оплата не прошла"}</h1>
                <span className={`whitespace-pre-line text-[#9FB3B8] text-md text-center ${!success ? "max-[481px]:text-[11px]" : "max-[481px]:text-[12px]"}`}>{success ? "Средства успешно зачисляются на ваш аккаунт.\nОбычно это занимает несколько минут." : "К сожалению, произошла ошибка при обработке платежа.\nПопробуйте еще раз или обратитесь в поддержку."}</span>
            </div>
            <div className="flex relative z-5">
                <div className="bg-[#0C161B] border-1 border-(--border) rounded-3xl p-10 max-[481px]:p-5 flex flex-col gap-5 max-[481px]:mx-5">
                    ...
                    <div className="flex gap-5 max-[481px]:gap-2">
                        {success ?
                        <Link href="/" className="text-center btn p-5 !rounded-3xl w-full font-(family-name:--manrope-semibold)">Перейти <span className="max-[481px]:whitespace-nowrap">на главную</span></Link>
                        :
                        ...
                        <Link href="/" className="text-center btn p-5 !rounded-3xl w-full font-(family-name:--manrope-semibold) !bg-none !shadow-none border-[#3A5056] border-1">Написать <span className="max-[481px]:whitespace-nowrap">в поддержку</span></Link>
                    </div>
                </div>
                <Image className="absolute right-10 -bottom-10 max-[481px]:hidden" width={success ? 500 : 420} height={success ? 500 : 420} src={success ? "/images/mika_ok.png" : "/images/mika_sad.png" } alt="mika" />
            </div>
        </section>
    )
}